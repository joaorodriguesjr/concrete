...
## Acceptance Criteria
- [x] The system does not feature a dedicated "Calculate" button for obtaining the final material quantities.
- [x] Every value change in any input field (via JavaScript `oninput` or `onchange` events) immediately triggers the calculation function.
- [x] The results panel on the screen updates instantly when valid inputs are given.
